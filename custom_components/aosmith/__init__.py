...
async def async_setup_entry(hass: HomeAssistant, entry: AOSmithConfigEntry) -> bool:
    ...
    try:
        ...
        _LOGGER.warning(
            "AO Smith: initial status fetch failed, will retry in background: %s", err
        )

    device_registry = dr.async_get(hass)
    for junction_id, aosmith_device in (status_coordinator.data or {}).items():
    ...
    energy_coordinator = AOSmithEnergyCoordinator(
        hass, entry, client, list(status_coordinator.data or {})
    )
    try:
        await energy_coordinator.async_config_entry_first_refresh()
    except UpdateFailed as err:
        _LOGGER.warning(
            "AO Smith: initial energy fetch failed, will retry in background: %s", err
        )
