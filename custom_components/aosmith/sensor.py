...
async def async_setup_entry(
    ...
    async_add_entities(
        AOSmithEnergyTodaySensorEntity(data.energy_coordinator, junction_id)
        for junction_id in data.status_coordinator.data
    )
...
class AOSmithEnergySensorEntity(AOSmithEnergyEntity, SensorEntity):
    """Class for the lifetime energy sensor entity."""
...
class AOSmithEnergyTodaySensorEntity(AOSmithEnergyEntity, SensorEntity):
    """Class for the today's energy usage sensor entity."""

    _attr_name = "Energy Usage Today"
    _attr_device_class = SensorDeviceClass.ENERGY
    _attr_state_class = SensorStateClass.TOTAL_INCREASING
    _attr_native_unit_of_measurement = UnitOfEnergy.KILO_WATT_HOUR
    ...
    def __init__(
        ...
        """Initialize the entity."""
        super().__init__(coordinator, junction_id)
        self._attr_unique_id = f"energy_usage_today_{junction_id}"

    @property
    def native_value(self) -> float | None:
        """Return today's energy usage from the history list."""
        today = date.today().isoformat()
        for entry in self.energy_data.history:
            if entry.date == today:
                return entry.energy_use_kwh
        return None
