...
class AOSmithEnergyEntity(AOSmithEntity[AOSmithEnergyCoordinator]):
    ...
    @property
    def energy_data(self) -> EnergyUseData:
        """Shortcut to get the full energy use data from the coordinator."""
        return self.coordinator.data[self.junction_id]

    @property
    def energy_usage(self) -> float:
        """Shortcut to get the lifetime energy usage from the coordinator data."""
        return self.energy_data.lifetime_kwh
