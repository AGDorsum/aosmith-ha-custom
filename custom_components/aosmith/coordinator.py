...
from py_aosmith.models import Device as AOSmithDevice, EnergyUseData
...
class AOSmithEnergyCoordinator(DataUpdateCoordinator[dict[str, EnergyUseData]]):
    ...
    async def _async_update_data(self) -> dict[str, EnergyUseData]:
        """Fetch latest data from the energy usage endpoint."""
        energy_data_by_junction_id: dict[str, EnergyUseData] = {}

        for junction_id in self.junction_ids:
            try:
                energy_use_data = await self.client.get_energy_use_data(junction_id)
            except AOSmithInvalidCredentialsException as err:
                raise ConfigEntryAuthFailed from err
            except AOSmithUnknownException as err:
                raise UpdateFailed(f"Error communicating with API: {err}") from err

            energy_data_by_junction_id[junction_id] = energy_use_data

        return energy_data_by_junction_id
